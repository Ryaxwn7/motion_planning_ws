    ...
    private:
        ...
        (bool saturate = false) {
            ...
            if (saturate)
                ...
                maxGridval = vmax / maxValue;
                
            for (int i = 0; i < grid_->size(); i++) {
                double vel = grid_->getCell(i).getValue() / maxValue; // [0,1]
                ...
                if (saturate)
                    if (vel < maxGridval)
                    {
                        ...
                        grid_->getCell(i).setVelocity(vel / maxGridval);

                    }
                    else
                        grid_->getCell(i).setVelocity(vmax);
                else
                    grid_->getCell(i).setVelocity(vel*vmax);
