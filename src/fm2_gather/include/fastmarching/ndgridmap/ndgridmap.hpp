    ...
    public:
     ...
     /**
       ...
       */
        ...
        nDGridMap<T, ndims>(const nDGridMap<T, ndims> & other)
        : leafsize_(other.getLeafSize()),
        ncells_(other.getNCells()),
        dimsize_(other.getDimSizes())
        {
        // 深拷贝 cells_，假设它是一个动态分配的数组或容器
        cells_.resize(ncells_, T()); // 调整大小以匹配 other.cells_
        std::copy(other.cells_.begin(), other.cells_.end(), cells_.begin());
        }

        nDGridMap<T, ndims>& operator=(const nDGridMap<T, ndims>& other) 
        {
        if (this != &other) { // 检查自赋值
            // 释放当前对象的资源（如果有需要）
            // 例如，如果 cells_ 是动态分配的内存，需要释放它
            // delete[] cells_; // 如果 cells_ 是动态分配的数组
    ...
    return *this; // 返回当前对象的引用
       ...
       */
       ...
       /**
        ...
        int getNCells() const {return ncells_;}
        ...
        std::array<int, ndims> getD() const {return d_;}
