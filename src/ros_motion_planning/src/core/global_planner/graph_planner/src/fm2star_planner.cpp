...
Description: FM2* 路径规划器（与 FM2 参数/速度场一致，搜索范围优化）
...
FM2Star_Planner::FM2Star_Planner(costmap_2d::Costmap2D* costmap) : GlobalPlanner(costmap) {
    ros::NodeHandle nh("~");
    loadParams(nh);
...
FM2Star_Planner::FM2Star_Planner(costmap_2d::Costmap2DROS* costmap_ros, costmap_2d::Costmap2D* costmap) : GlobalPlanner(costmap) {
    ros::NodeHandle nh("~");
    loadParams(nh);
    setupDynamicReconfigure(nh);
...
FM2Star_Planner::FM2Star_Planner(costmap_2d::Costmap2DROS* costmap_ros,
...
void FM2Star_Planner::loadParams(const ros::NodeHandle& nh) {
    ...
    nh.param("fm2/use_star_heuristic", use_star_heuristic_, true);
...
void FM2Star_Planner::setupDynamicReconfigure(const ros::NodeHandle& nh) {
    ros::NodeHandle fm2_nh(nh, "fm2");
    dyn_server_ = new dynamic_reconfigure::Server<graph_planner::FM2PlannerConfig>(fm2_nh);
    dyn_cb_ = boost::bind(&FM2Star_Planner::reconfigureCB, this, _1, _2);
    dyn_server_->setCallback(dyn_cb_);
}

void FM2Star_Planner::reconfigureCB(graph_planner::FM2PlannerConfig& config, uint32_t level) {
    if (!setup_) {
    ...
    use_star_heuristic_ = config.use_star_heuristic;
    ...
    if (debug_log_) {
        ROS_INFO("FM2*: Reconfigure v_max=%.3f alpha=%.3f dmax=%.3f radius=%.3f mode=%d k=%.3f b=%.3f gather=%s star=%s dyn_unc=%s cluster_track=%s full_refresh=%s cluster_min=%d track_r=%.2f ttl=%.2f static_reject=%d",
                 ...
                 use_star_heuristic_ ? "true" : "false",
...
void FM2Star_Planner::dumpVelocityMapCsv(const std::string& path) {
...
void FM2Star_Planner::initializeFastMarching(costmap_2d::Costmap2D* costmap) {
    ndims_ = 2;
...
void FM2Star_Planner::initializeFastMarchingROS(costmap_2d::Costmap2DROS* costmap_ros, costmap_2d::Costmap2D* costmap) {
    ndims_ = 2;
    nx_ = costmap->getSizeInCellsX();
    ny_ = costmap->getSizeInCellsY();
    ns_ = nx_ * ny_;
...
bool FM2Star_Planner::recoverGrid() {
    int i = 0;
...
bool FM2Star_Planner::resetGrid() {
    for (int i = 0; i < ns_; ++i)
    {
...
bool FM2Star_Planner::updateGrid(costmap_2d::Costmap2D* costmap)
{
    if (!resetGrid()) {
...
void FM2Star_Planner::invalidateVelocityCache() {
...
void FM2Star_Planner::rebuildStaticObstacleProximityMask() {
...
bool FM2Star_Planner::isNearStaticObstacle(const int idx) const {
    if (idx < 0 || idx >= ns_) {
        return false;
    }
...
bool FM2Star_Planner::isDynamicObstacleCost(const unsigned char cost) const {
...
void FM2Star_Planner::collectDynamicObstacleCells(
...
void FM2Star_Planner::extractDynamicObstacleClusters(
    const unsigned char* map_data,
...
void FM2Star_Planner::updateDynamicObstacleTracks(
...
double FM2Star_Planner::getVelocityReference(const std::vector<double>& velocity_map) const {
...
const std::vector<FM2Star_Planner::UncertaintyKernelCell>&
FM2Star_Planner::getUncertaintyKernel(int hard_cells, int soft_cells) {
...
int FM2Star_Planner::applyUncertaintyKernelAt(
    const int cx,
    const int cy,
...
bool FM2Star_Planner::ensureBaseVelocityMap(const std::vector<int>& init_point, const int goal_idx) {
    ...
    // FM2* 的速度映射与 FM2 一致，这里只复用 FM2 的速度场计算。
...
bool FM2Star_Planner::applyDynamicObstacleUncertainty(
...
bool FM2Star_Planner::plan(const Node& start, const Node& goal, std::vector<Node>& path , std::vector<Node>& expand) {

    path.clear();
    expand.clear();
    const int start_idx = grid2Index(start.x(), start.y());
    ...
    if (use_gather_style_) {
        init_point = {goal_idx};
        wave_goal_idx = start_idx;
    } else {
        init_point = {start_idx};
        wave_goal_idx = goal_idx;
    }
    ...
    if (!ensureBaseVelocityMap(init_point, wave_goal_idx)) {
        return false;
    }
    ...
    FastMarching2Star<nDGridMap<FMCell, 2>> fm2;
    fm2.setVelocityScale(v_max_);
    fm2.setVelocityProfile(velocity_alpha_, velocity_dmax_);
    fm2.setRobotRadius(robot_radius_);
    fm2.setVelocityMode(velocity_mode_);
    fm2.setVelocitySigmoid(velocity_sigmoid_k_, velocity_sigmoid_b_);
    fm2.setEnvironment(&grid_);
    fm2.setInitialAndGoalPoints(init_point, fm2_sources_, wave_goal_idx);

    fm2.computeFM2Star_v(runtime_velocity_map, use_star_heuristic_);
    ...
    fm2.computePath(&fm2_path, &path_velocity);
    if (fm2_path.empty()) {
...
bool FM2Star_Planner::plan(costmap_2d::Costmap2D* costmap, const Node& start, const Node& goal, std::vector<Node>& path, std::vector<Node>& expand)
{
    path.clear();
    expand.clear();
    const int start_idx = grid2Index(start.x(), start.y());
