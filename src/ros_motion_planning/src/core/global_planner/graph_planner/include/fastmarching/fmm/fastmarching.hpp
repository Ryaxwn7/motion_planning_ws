...
template < class grid_t, class heap_t = FMDaryHeap<FMCell> >  class FastMarching {
    ...
    public:
        ...
        (grid_t * g) {
            ...
            // std::cout << "[fmm]Fast Marching initialized with " << grid_->size() << " cells." << std::endl;
        }
        ...
        (const std::vector<int> & init_points, int goal_idx = -1) {
            ...
            goal_idx_ = goal_idx;
            // std::cout << "[fmm]Initial point size: " << init_points_.size() << std::endl;
           
            // std::cout << "[fmm]Goal point: " << goal_idx_ << std::endl; 
            for (const int &i: init_points) { //&i 使用引用避免拷贝提高效率
                grid_->getCell(i).setArrivalTime(0);
                grid_->getCell(i).setState(FMState::FROZEN);
                // std::cout << "[fmm]Cell " << i << " set to frozen." << std::endl;
            }
         ...
         */
        virtual void plotFrozenMap
        (const bool flipY = 1) {
            // TODO: image checking: B/W, correct reading, etc.
            std::array<int,2> d = grid_->getDimSizes();
            cimg_library::CImg<bool> img(d[0],d[1],1,1,0);
            if (flipY)
                // Filling the image flipping Y dim. We want now top left to be the (0,0).
                cimg_forXY(img,x,y) { img(x,y) = grid_->getCell(img.width()*(img.height()-y-1)+x).getState() == FMState::FROZEN; }	
            else 
                cimg_forXY(img,x,y) { img(x,y) = grid_->getCell(img.width()*y+x).getState() == FMState::FROZEN; }	
                
            img.display("Frozen map", false);
        }
        virtual void plotOpenMap
        (const bool flipY = 1) {
            // TODO: image checking: B/W, correct reading, etc.
            std::array<int,2> d = grid_->getDimSizes();
            cimg_library::CImg<bool> img(d[0],d[1],1,1,0);
            if (flipY)
                // Filling the image flipping Y dim. We want now top left to be the (0,0).
                cimg_forXY(img,x,y) { img(x,y) = grid_->getCell(img.width()*(img.height()-y-1)+x).getState() == FMState::OPEN; }	
            else 
                cimg_forXY(img,x,y) { img(x,y) = grid_->getCell(img.width()*y+x).getState() == FMState::OPEN; }	
                
            img.display("Open map", false);
        }
        virtual void plotOccMap
        (const bool flipY = 1) {
            // TODO: image checking: B/W, correct reading, etc.
            std::array<int,2> d = grid_->getDimSizes();
            cimg_library::CImg<bool> img(d[0],d[1],1,1,0);
            if (flipY)
                // Filling the image flipping Y dim. We want now top left to be the (0,0).
                cimg_forXY(img,x,y) { img(x,y) = grid_->getCell(img.width()*(img.height()-y-1)+x).isOccupied(); }	
            else 
                cimg_forXY(img,x,y) { img(x,y) = grid_->getCell(img.width()*y+x).isOccupied();}	
        ...
        () {
            ...
            for (int &i: init_points_) { // For each initial point
                ...
                // std::cout << "[fastmarching.hpp-init] init_points_:" << i <<"n_neighs:"<< n_neighs << std::endl;
                for (int s = 0; s < n_neighs; ++s){  // For each neighbor
                    ...
                    if ((grid_->getCell(j).getState() == FMState::FROZEN) || grid_->getCell(j).isOccupied() || grid_->getCell(j).getVelocity() == 0 ) // If Frozen or obstacle
                        {
                            // std::cout << "[fastmarching.hpp-init] Cell " << j << " grid_->getCell(j).isOccupied() " << grid_->getCell(j).isOccupied() << " grid_->getCell(j).getVelocity() " << grid_->getCell(j).getVelocity() << " State {1-Frozen}"<<((grid_->getCell(j).getState() == FMState::FROZEN)? 1:0) << " State {1-Open}"<< ((grid_->getCell(j).getState() == FMState::OPEN)? 1:0) << " State {1-Narrow}"<< ((grid_->getCell(j).getState() == FMState::NARROW)? 1:0)  << std::endl;
                            continue;}
                    else {
                        // std::cout << "[fastmarching.hpp-init] Expanding cell increasing narrow band " << j << std::endl;
        ...
        (const int & idx) {
            ...
            // std::cout<< "[Eikonal]grid_->getLeafSize() : "<< grid_->getLeafSize() << " grid_->getCell(idx).getVelocity() : "<< grid_->getCell(idx).getVelocity() << std::endl;
            // std::cout<< "[Eikonal]a: "<< a << " b: "<< b << " c: "<< c << std::endl;
        ...
        () {
            ...
            std::cout << "[fmm]Starting Fast Marching Method. narrows_band_ size: " << narrow_band_.size() << std::endl;
            ...
            while (!stopWavePropagation && narrow_band_.size() > 0) 
            {
                ...
                for (int s = 0; s < n_neighs; ++s) {
                    ...
                    if ((grid_->getCell(j).getState() == FMState::FROZEN) || grid_->getCell(j).isOccupied())
                        {
                            // std::cout << "Cell " << j << " is frozen or occupied. Skipping." << std::endl;
                            continue;
                            }
                    else {
                        // std::cout << "Expanding cell " << j << std::endl;
                        double new_arrival_time = solveEikonal(j);
                        // 更新NARROW状态的点
